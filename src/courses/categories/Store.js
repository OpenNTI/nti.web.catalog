...
export default class CourseListStore extends SearchablePagedStore {
	...
	async loadSearchTerm(term) {
		...
		const searchItems = await service.getBatch(collection.href, {
	...
	async loadInitial() {
		...
		try {
			...
			categories = await this.parseRaw(
				await service.get(
					UrlUtils.appendQueryParams(href, {
			...
			if (
				...
				categories.Items[0].Name === otherGroup
			) {
				href = href + '/' + otherGroup;
				categories = await service.getBatch(href, {
					batchSize: Constants.BATCH_SIZE,
					batchStart: 0,
				});
				categories = {
		...
		try {
			carousel = await this.parseRaw(
				await service.get(getLink(links, 'Featured'))
			);
		} catch (e) {
			carousel = { Items: [] };
		}
