...
describe('Redeem', () => {
	...
	test('Test redeem with empty code', async () => {
		...
		expect(container.querySelector('.error-redeem p').textContent).toEqual(
			errorMessage.empty
		);
	});
	...
	test('Test redeem with enrolled code', async () => {
		...
		return waitFor(() =>
			expect(ref.current.state.errorMessage).toEqual(
	...
	test('Test redeem success', async () => {
		...
		return waitFor(() => expect(ref.current.state.error).toEqual(false));
	});
