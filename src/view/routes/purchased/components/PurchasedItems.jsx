...
const t = scoped('nti-catalog.view.routes.purchased.components.PurchasedItems', {
...
export default function PurchasedItems () {
	const resolver = useResolver(async () => {
		...
		const purchasedHref = service?.getCollection('Purchased', 'Catalog')?.href;

		const purchased = await service.getBatch(purchasedHref, {batchSize: BatchSize, batchStart: 0});
